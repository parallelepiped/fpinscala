...
object Par {
  ...
  // that records the amount of time spent evaluating `af`, then subtracts that time from the available time allocated for evaluating `bf`. And vice-versa?
  ...
  //ex 7.6
  def parFilterQ[A](as: List[A])(f: A => Boolean): Par[List[A]] = {
    ???
  }

  //ex 7.6
  def parFilter[A](as: List[Par[A]])(f: A => Boolean): Par[List[A]] = {
    Par.map(Par.sequence(as))(_.filter(f))   // really?
  }
  ...
  class ParOps[A](p: Par[A]) {
    def map[B](f: A => B): Par[B] = Par.map(this.p)(f)
  }
}
